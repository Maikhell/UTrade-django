...
function showUserDetails(firstName, lastName, studentNo, course, section, corUrl) {
    const img = document.getElementById('cor_image');

    if (!img) {
        console.error('cor_image not found in DOM');
        return;
    }

    if (corUrl && corUrl.trim() !== '') {
        img.src = corUrl;
    } else {
        img.src = '';
        img.alt = 'No COR uploaded';
    }

    const modal = new bootstrap.Modal(document.getElementById('corModal'));
    modal.show();
}
